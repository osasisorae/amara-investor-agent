...
function buildDiasporaTransferAnswer(): string {
  ...
  const transferSentence =
    section
      .split('\n')
      .find((line) => line.includes('Diaspora investors transfer funds')) || '';
  const returnSentence =
    section
      .split('\n')
      .find((line) => line.includes('Returns are distributed in Naira quarterly')) ||
    '';
  const fxSentence =
    section
      .split('\n')
      .find((line) => line.includes('exchange rate risk')) || '';

  return [
    transferSentence.replace(/^\*\*.*\*\*\s*/, '').trim(),
    returnSentence.trim(),
    fxSentence.replace(/^\*\*Exchange rate context:\*\*\s*/, '').trim(),
  ]
    .filter(Boolean)
    .join(' ');
}
